...
public class Test1Class extends AbstractBuilderTest {
	...
	public void test_1_1_ClassName() throws IOException {
		...
		db.withClasses(Test1Class.class).show(glyphsWithStereotypes(db));
		renderDiagram(db.getDiagram());
	}

	private Format glyphsWithStereotypes(DiagramBuilder db) {
		return ADLFormat.composesAsContainedGlyph(db.getRepresentation(), 
				ADLFormat.typeAsStereotype(db.getRepresentation(), 
				ADLFormat.asConnectionWithBody(db.getRepresentation())), null);
	}
	...
	public void test_1_2_ClassExtendsAsSymbol() throws IOException {
		DiagramBuilder db = createBuilder();
		db.withClasses(Test1Class.class)
			.show(glyphsWithStereotypes(db))
			.withSuperClasses(null)
			.show(db.asSymbols());
		renderDiagram(db.getDiagram());
	}
	...
	public void test_1_4_ClassInterfacesAsArrows() throws IOException {
		DiagramBuilder db = createBuilder();
		db.withClasses(Test1Class.class, DummyClass.class).show(db.asConnectedGlyphs()).withInterfaces(null, false).show(db.asConnectedGlyphs());
		renderDiagram(db.getDiagram());
	}
