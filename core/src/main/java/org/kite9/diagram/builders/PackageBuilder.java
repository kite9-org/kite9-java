...
public class PackageBuilder extends AnnotatedElementBuilder<Package> {
	...
	for (Class<?> c : classes) {
	    ...
	    if (t!=null) {
		ties2.add(new Tie(createNewSubjectNounPart(t), HasRelationship.CLASS, createNoun(c)));
	    }
	}
	...
	for (Tie t : ties) {
		...
		if ((f==null) || (f.accept(c))) {
		    ties2.add(new Tie(sub, HasRelationship.CLASS, createNoun(c)));
		}
	    }
	}
	ClassBuilder ch = new ClassBuilder(ties2, model, a);
    ...
    public PackageBuilder withDependencies(Filter<? super Package> f) {
    	...
    	for (Tie t : ties) {
    		...
    		if ((f==null) || (f.accept(p2))) {
    		    ties2.add(new Tie(sub, Relationship.REQUIRES, createNoun(p2)));
    		}
    	    }
    	}
