...
import org.kite9.diagram.builders.krmodel.noun.AnnotatedNounPartImpl;
...
public class TypeNounHelper {
	...
	public TypeNounHelper(IdHelper idHelper, Aliaser aliaser) {
	...
	public class MapUnraveller implements Unraveller {
		...
		public NounPart unravel(Type to, Aliaser a) {
				...
				return new ProjectStaticSimpleNoun(idHelper.getId(raw), raw, aliaser.getObjectAlias(raw), disambiguation);
		}
	...
	public class BasicUnraveller implements Unraveller {
		...
		public NounPart unravel(Type t, Aliaser a) {
			return new ProjectStaticSimpleNoun(idHelper.getId(t), (Class<?>) t, aliaser.getObjectAlias(t));
		}

	}


	public class TypeVariableUnraveller implements Unraveller {
		...
		public NounPart unravel(Type t, Aliaser a) {
			String id = idHelper.getId(t);
			String alias = aliaser.getObjectAlias(t);
			return new ProjectStaticSimpleNoun(id, t, alias);
		}

	}
	
	public class ParameterizedTypeUnraveller implements Unraveller {
		...
		public NounPart unravel(Type t, Aliaser a) {
			...
			String id = idHelper.getId(rawType);
			String alias = aliaser.getObjectAlias(rawType);
			return new ProjectStaticSimpleNoun(id, rawType, alias);
		}
